...
else (the Zenon node connection, Nostr relays, and the connected wallet
extension) is called directly from the browser. Cloudflare Pages is the
...
The public testnet (chain `73404`) has no faucet yet, so a testnet
deployment is mostly useful for reviewing the UI, not for swaps. To run one
anyway, either:
